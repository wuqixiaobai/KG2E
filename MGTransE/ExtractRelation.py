...
def Extract_FB(dirTrain="data/FB15k/train.txt"):
    result = {
    }  # key is Relation, value is a list which has a special realtion between entity
    sp = "\t"
    with open(dirTrain) as file:
        line = file.readline()
        while line:
...
if __name__ == '__main__':
    ...
    for keys in Relation.keys():
        listFB = FB_FB[keys]
        print(
            "Has processing the %d(All is 1345) And the size of Relation is %d "
            % (index, len(listFB)))
        Relation_Result = []
        for entity in listFB:
            if entity.split("\t")[0] in FB_DB.keys() and entity.split(
                    "\t")[1] in FB_DB.keys():
                ...
                #print(sparqlString)
                try:
                    result = SparqlQuery("http://dbpedia.org/sparql/",
                                         sparqlString)
                except:
                    continue
                if result != []:
                    ...
                    for relation in result:
                        DBRelation.append(DBEntity1 + "\t" + DBEntity2 + "\t" +
                                          result)
            else:
                continue
        if Relation_Result != []:
