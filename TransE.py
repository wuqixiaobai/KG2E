...
class TransE:
    def __init__(self, entityList, relationList, tripleList, margin = 1, learingRate = 0.0001, dim = 10, L1 = True):
        self.margin = margin
    ...
    def transE(self, cI = 20):
        ...
        for cycleIndex in range(cI):
            bachsize= 1000
            number_bachsize=int(533490/bachsize)
            for _ in range(number_bachsize):
                Sbatch = self.getSample(bachsize) #SGD的Batch
...
if __name__ == '__main__':
    dirEntity = "data/DB_FB30k/entity2id.txt"
    entityIdNum, entityList = openDetailsAndId(dirEntity)
    dirRelation = "data/DB_FB30k/relation2id.txt"
    relationIdNum, relationList = openDetailsAndId(dirRelation)
    dirTrain = "data/DB_FB30k/train.txt"
    tripleNum, tripleList = openTrain(dirTrain)
    print(tripleNum)
    print("打开TransE")
    transE = TransE(entityList,relationList,tripleList, margin=2, dim = 50, L1 = True)
    print("TranE初始化")
    transE.initialize()
    transE.transE(1000)
    transE.writeRelationVector("data/DB_FB30k/relationVector.txt")
    transE.writeEntilyVector("data/DB_FB30k/entityVector.txt")
    print("开始测试")
    dirEntityTest =  "data/DB_FB30k/test_Entity.txt"
    EntityTest = test.openD(dirEntityTest)
    print("Read Entity")
    dirRelationTest = "data/DB_FB30k/test_Relation.txt"
    RelationTest = test.openD(dirRelationTest)
    print("Read Relation")
    dirEntityVector = "data/DB_FB30k/entityVector.txt"
    entityVectorList, entityList = test.loadData(dirEntityVector)
    print("Read EntitVector")
    dirRelationVector = "data/DB_FB30k/relationVector.txt"
    ...
    testHeadRaw.writeRank("data/DB_FB30k/" + "testHeadRaw" + ".txt")
    """
    ...
    testTailRaw.writeRank("data/DB_FB30k/" + "testTailRaw" + ".txt")
    print("Done")
